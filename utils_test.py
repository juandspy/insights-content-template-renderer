...
def test_get_reported_error_key():
    report = test_data["report_data"]["reports"]["5d5892d3-1f74-4ccf-91af-548dfc9767aa"]["reports"][0].copy()
...
def test_render_resolution():
    ...
    result = 'Red Hat recommends that you configure your nodes to meet the minimum resource requirements.' \
        'Make sure that:1. Node foo1 (undefined) * Has enough memory, minimum requirement is 16. Currently' \
...
def test_render_reason():
    ...
    assert utils.render_reason(rule_content, report) == 'Node not meeting the minimum requirements:1. foo1 * ' \
    'Roles: undefined * Minimum memory requirement is 16, but the node is configured with 8.16.'
...
def test_render_report():
    ...
    result = {
        ...
        'reason': 'Node not meeting the minimum requirements:1. foo1 * Roles: undefined * ' \
            ...
            '5d5892d3-1f74-4ccf-91af-548dfc9767aa': [
                {
                    ...
                    'resolution': 'Red Hat recommends that you configure your nodes to meet the minimum ' \
                                  'resource requirements.Make sure that:1. Node foo1 (undefined) * Has ' \
                                  'enough memory, minimum requirement is 16. Currently its only configured with 8.16GB.',
                    'reason': 'Node not meeting the minimum requirements:1. foo1 * Roles: undefined * Minimum memory' \
                              ' requirement is 16, but the node is configured with 8.16.'
                }, 
                {
                    'rule_id': 'samples_op_failed_image_import_check', 
                    'error_key': 'SAMPLES_FAILED_IMAGE_IMPORT_ERR', 
                    'resolution': 'Red Hat recommends that you to follow these steps:1. Fix 1, Try running:~~~# oc ' \
                                  'import-image <for the ImageStream(s) in question>~~~1. Fix 2, Try running:~~~# oc ' \
                                  'delete configs.samples cluster~~~', 
                    'reason': 'Due to a temporary hiccup talking to the Red Hat registry the openshift-samples failed ' \
                              'to import some of the imagestreams.Source of the issue:**Cluster-operator:**  ' \
                              '**openshift-samples**- *Condition:* Degraded- *Reason:* FailedImageImports- *Message:* ' \
                ...
                }, 
                {
                    ...
                    'resolution': 'Red Hat recommends that you to follow steps in the KCS article. * [Authentication ' \
                                  'operator Degraded with Reason `WellKnownEndpointDegradedError`]' \
